...
static u32 HandleRegionMapInput(struct Pokenav_RegionMapMenu *state)
{
    ...
    case MAP_INPUT_R_BUTTON:
        if ((regionMap->mapSecType == MAPSECTYPE_CITY_CANFLY || regionMap->mapSecType == MAPSECTYPE_BATTLE_FRONTIER) //Ttime100 added
...
void UpdateRegionMapHelpBarText(void)
{
    ...
    if ((regionMap->mapSecType == MAPSECTYPE_CITY_CANFLY || regionMap->mapSecType == MAPSECTYPE_BATTLE_FRONTIER) //Ttime100 fixed
        && FlagGet(OW_FLAG_POKE_RIDER) && Overworld_MapTypeAllowsTeleportAndFly(gMapHeader.mapType) == TRUE)
    {
        if (IsRegionMapZoomed())
