...
The _Vs. Seeker_ is a Key Item that is used to battle Trainers that the player has battled previously.
...
For developers, Vs. Seeker functionality is enabled by setting `I_VS_SEEKER_CHARGING` to `TRUE`.

For players, `ITEM_VS_SEEKER` can only be used outside of battle. It can be used from the bag or registered to be used from the field.
...
### How do developers implement rematches with the Vs. Seeker?

For existing `pokemerald` Trainers, no extra work is required. With the exception of Wally, Gym Leaders and Elite Four, all of the rematchable Trainers in Emerald will work with the Vs. Seeker without any changes.

For new Trainers, each of the rematches for the Trainer must be defined as seperate Trainers in `src/data/trainers.party` and `include/constants/opponents`. For example, `TRAINER_CALVIN_1` also has `TRAINER_CALVIN_2`,`TRAINER_CALVIN_3`,`TRAINER_CALVIN_4`, and `TRAINER_CALVIN_5`.
...
> [!WARNING]
> **Rematch IDs should be placed** _before `REMATCH_WALLY_VR`. Trainers below that are treated as "special Trainers" that are not triggered by the Vs. Seeker._
