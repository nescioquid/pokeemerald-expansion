...
To run specific tests, such as those for Spikes, use:

```console
make check TESTS="Spikes"
```

To build a ROM (pokemerald-test.elf) that can be opened in mGBA to view specific tests, such as those for Spikes, use:
...
As a concrete example, to manually test `EFFECT_PARALYZE`, as in the effect of Stun Spore, you might:
...
The `ASSUMPTIONS` block documents that Stun Spore has `EFFECT_PARALYZE`. If Stun Spore did not have that effect it would cause the tests in the file to be skipped. We write our tests like this so that developers can change the effects of moves without causing tests to fail.

`SINGLE_BATTLE_TEST` defines the name of the test. Related tests should start with the same prefix, such as Stun Spore tests should start with "Stun Spore", this allows just the Stun Spore-related tests to be run with:
...
`PARAMETRIZE` causes a test to run multiple times, once per `PARAMETRIZE` block (so once with `raiseAttack = FALSE` and once with `raiseAttack = TRUE`).
...
You might notice that all the tests check the outputs the player could see rather than the internal battle state. For instance, the Meditate test could have used `gBattleMons[B_POSITION_OPPONENT_LEFT].hp` instead of using `HP_BAR` to capture the damage. This is a deliberate choice, by checking what the player can observe the tests are more robust to refactoring, so if `gBattleMons` got moved into `gBattleStruct` then any test that used it would need to be updated.

>[!NOTE]
> _The overworld is not available, so it is only possible to test commands which don't affect the overworld itself. For example, `givemon` can be tested because it only alters `gPlayerParty`, but `addobject` cannot because it affects object events (which aren't loaded)._
...
Causes the test to be skipped if `cond` is false. Used to document any prerequisites of the test, such as to test Burn reducing the Attack of a Pokémon we can observe the damage of a physical attack with and without the burn. To document that this test assumes the attack is physical we can use `ASSUME(GetMoveCategory(MOVE_WHATEVER) == DAMAGE_CATEGORY_PHYSICAL);`.
...
Should be placed immediately after any `#includes` and contain any `ASSUME` statements which should apply to the whole file. To test `EFFECT_POISON_HIT` for example, we need to choose a move with that effect, and if we chose to use Poison Sting in every test then the top of `move_effect_poison_hit.c` would be:
...
- `DOUBLE_BATTLE_TEST(name, results...)`,
- `MULTI_BATTLE_TEST(name, results...)`,
- `TWO_VS_ONE_BATTLE_TEST(name, results...)`, and
- `ONE_VS_TWO_BATTLE_TEST(name, results...)`
...
- `AI_DOUBLE_BATTLE_TEST(name, results...)`,
- `AI_MULTI_BATTLE_TEST(name, results...)`,
- `AI_TWO_VS_ONE_BATTLE_TEST(name, results...)`, and
- `AI_ONE_VS_TWO_BATTLE_TEST(name, results...)`

Define battles where opponent mons are controlled by AI, the same that runs when battling regular Trainers. The flags for AI should be specified by the `AI_FLAGS` command. The rules remain the same as with the `SINGLE`, `DOUBLE`, `MULTI`, `TWO_VS_ONE`, and `ONE_VS_TWO` battle tests with some differences:
...
`KNOWN_FAILING;` marks a test as not passing due to a bug. If there is an issue number associated with the bug it should be included in a comment. If the test passes the developer will be notified to remove `KNOWN_FAILING`:
...
Runs a test multiple times. `i` will be set to which parameter is running, and `results` will contain an entry for each parameter:
...
This mode of PASSES_RANDOMLY makes the tests run very slowly and should be avoided where possible. If the mechanic you are testing is missing its tag, you should add it.
...
Explicitly sets the RNG seed. Try to avoid using, as it is a very fragile tool:
...
Sets the specified flag. Can currently only set one flag at a time. Cleared between parameters and at the end of the test:
...
Add the species to the player's or opponent's party respectively. The Pokémon can be further customized with the following functions:
...
- if Speed is specified for any Pokémon then it must be specified for all Pokémon.
- if Moves is specified then MOVE will not automatically add moves to the moveset.
...
- `TWO_VS_ONE_BATTLE_TEST`,
- `AI_TWO_VS_ONE_BATTLE_TEST`,
- `ONE_VS_TWO_BATTLE_TEST`, or
- `AI_ONE_VS_TWO_BATTLE_TEST`

the below must be used instead of `PLAYER(species)` and `OPPONENT(species)`:

- `MULTI_PLAYER(species)`, 
- `MULTI_PARTNER(species)`, 
- `MULTI_OPPONENT_A(species)`, and
- `MULTI_OPPONENT_B(species)`

Add the species to the player's, player partner's, opponent A's, or opponent B's party, respectively.
...
Specifies additional AI flags to be applied to specific battlers (battler 0/1/2/3). Has use only for AI tests. Must be used strictly after `AI_FLAGS(flags)`, which overwrites all existing flags.

Example: `BATTLER_AI_FLAGS(3, AI_FLAG_RISKY)` used after `AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT)` will set `AI_FLAG_RISKY` to only `battler3` (Opponent B), in addition to the flags set by `AI_FLAGS`.
...
Groups the choices made by the battlers on a single turn. If Speeds have not been explicitly specified then the order of the `MOVE` commands in the `TURN` will be used to infer the Speeds of the Pokémon:
...
Used when the battler chooses Fight. Either the move ID (such as `MOVE_TACKLE`) or move slot must be specified.
...
- `allowed: FALSE` is used to reject an illegal move, such as a Disabled one.
...
Used when the battler chooses Fight and then their move is chosen for them, like when affected by Encore.
...
Used when the battler cannot choose an action, such as when locked into Thrash.
...
Used when the battler chooses to switch to another Pokémon but not via Switch, such as after fainting or due to a U-turn.
...
Used when the battler chooses to use an item from the Bag. The item ID (such as ITEM_POTION) must be specified, and party index and move slot if applicable:
...
Contains an abridged description of the UI during the `THEN`. The order of the description must match too:
...
Causes the test to fail if the battler's ability pop-up is not shown. If specified, ability is the ability shown in the pop-up.
...
Causes the test to fail if the animation does not play.

A common use of this command is to check if a move was successful:
...
If `exp:` is used, causes the test to fail if that amount of experience is not gained:

```
     EXPERIENCE_BAR(player, exp: 0);
```

If `captureGainedExp:` is used, causes the test to fail if the Experience bar does not change, and then writes that change to the pointer:
...
> [!NOTE]
> _Due to nature of tests, this command is only usable in `WILD_BATTLE_TEST` and will fail elsewhere._

### `HP_BAR`

`HP_BAR(battler, [damage: | hp: | captureDamage: | captureHP:])`
If `hp:` or `damage:` are used, causes the test to fail if that amount of damage is not dealt:

```
     HP_BAR(player, hp: 0);
```

If `captureDamage:` or `captureHP:` are used, causes the test to fail if the HP bar does not change, and then writes that change to the pointer:
...
Causes the test to fail if the message in pattern is not displayed. Spaces in pattern match newlines (\n, \l, and \p) in the message.

Often used to check that a battler took its turn but it failed:
...
If the expected status icon is parametrized the corresponding `STATUS1` constant can be provided:
...
> [!NOTE]
> _If this condition fails, the viewable ROM freezes at the NOT command._

> [!WARNING]
> **`NOT` is an alias of `NONE_OF`**_, so it behaves surprisingly when applied to multiple commands wrapped in braces._
...
Causes the test to fail if a and b compare incorrectly:
...
Causes the test to fail if `a*m != b` (within a threshold):
...
Returns a pointer to a compiled overworld script. Cannot be used to initialize global `const` data, although the pointer _IS_ to `const` data.

Note that each script command must be followed by a ;, for example:
